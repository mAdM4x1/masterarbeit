data <- read.csv("C:/Users/Florian/OneDrive - Universität Leipzig/Masterarbeit/02_Arbeit/Umfragen/Umfrageauswertung/survey_257533_R_data_file.csv", quote = "'\"", na.strings=c("", "\"\""), stringsAsFactors=FALSE, fileEncoding="UTF-8-BOM")
...
plot_question <- function(data, question_index, bar_color) {
  ...
  p <- ggplot(counts, aes_string(x = variable_name, y = "n")) +
    ...
    labs(title = wrap_text(paste("Antwortverteilung für die Frage:", variable_label), text_width),
         subtitle = paste("Gesamtanzahl der Antworten:", total_responses, 
  ...
  print(p)
  export_plot(p, paste0("Diagramme/", variable_name, "-", split_text(variable_label), ".png"))
}
...
p <- ggplot(counts, aes(x = !!sym(variable_name), y = n)) +
  geom_bar(stat = "identity", fill = bar_color) +
  labs(title = wrap_text(paste("Letzte Seite"), text_width),
       subtitle = paste("Gesamtanzahl der Antworten:", total_responses, 
